...
// TokenManager caches an STS token issued for a fixed (role,name,bucket)
// triple and refreshes it transparently before expiry. It is safe for
// concurrent use and is intended for long-lived controller-internal
// callers (OSS mc alias setup, APIG SDK client) that repeatedly ask for
// fresh credentials.
//
// Typical usage:
//
...
func (m *TokenManager) Token(ctx context.Context) (*IssueResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("refresh %s token: %w", m.req.Role, err)
	}
	m.cached = tok
