...
func (i *Initializer) initGatewayRoutes(ctx context.Context) error {
	...
	if cfg.LLMAPIKey != "" {
		...
		// Reconcilers (via AuthorizeAIRoutes). This separation of
		// ownership avoids the restart-time race where the initializer
		// would otherwise re-declare an empty allowedConsumers list and
		// transiently lock out the Manager/Workers.
