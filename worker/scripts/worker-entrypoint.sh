...
(
    while true; do
        sleep 300
        ensure_mc_credentials 2>/dev/null || true
        mc cp "${HICLAW_STORAGE_PREFIX}/agents/${WORKER_NAME}/openclaw.json" /tmp/openclaw-remote.json >/dev/null 2>/dev/null || true
        merge_openclaw_config /tmp/openclaw-remote.json "${WORKSPACE}/openclaw.json"
        rm -f /tmp/openclaw-remote.json
        mc cp "${HICLAW_STORAGE_PREFIX}/agents/${WORKER_NAME}/config/mcporter.json" "${WORKSPACE}/config/mcporter.json" >/dev/null 2>/dev/null || true
